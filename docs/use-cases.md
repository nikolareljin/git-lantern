...
6. **Clone destination**: Repos are cloned into namespace directories by default, so `my-namespace/my-repo` becomes `my-namespace/my-repo` under your workspace root. Choose the flat layout when you explicitly want the repository cloned directly into the root as `my-repo`.
