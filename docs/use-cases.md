...
6. **Clone destination**: Repos are cloned into your workspace root using the repo basename when it is available, so `my-namespace/my-repo` becomes `my-repo`; if that basename is already taken, Lantern falls back to an encoded namespaced path such as `my-namespace%2Fmy-repo`
...
- You can still narrow or override the scope with `--repos` or the TUI checklist. In TUI latest-refresh selection, actionable repos are prechecked but you can add or remove repos before apply.
